...
export default function AllStoresClient({ initialStores }: { initialStores: Store[] }) {
  ...
  useLayoutEffect(() => {
    setDirectoryVisibleCount(Math.min(DIRECTORY_BATCH_SIZE, filteredStores.length));
  }, [activeCategory, searchQuery, orderedStores.length, user?.id, filteredStores.length]);

  const visibleDirectoryStores = useMemo(
    () => filteredStores.slice(0, directoryVisibleCount),
    [filteredStores, directoryVisibleCount]
  );

  const hasMoreDirectory = directoryVisibleCount < filteredStores.length;
  ...
  const directoryCatIdx = useMemo(
    () => computeCategoryIndices(visibleDirectoryStores),
    [visibleDirectoryStores]
  );
        ...
        {filteredStores.length ? (
          <div className="space-y-10">
            ...
            <div>
              ...
              {renderResponsiveStoreGrid(visibleDirectoryStores, directoryCatIdx)}
              {hasMoreDirectory ? (

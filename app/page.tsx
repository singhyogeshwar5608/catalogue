  ...
  useEffect(() => {
    const fetchStores = async () => {
      try {
        ...
        console.log('All stores from API:', allStores);
        console.log('Store count:', allStores.length);
        allStores.forEach((store, index) => {
          console.log(`Store ${index + 1}:`, store.name, store.id, store.location);
        });
        setStores(allStores);
      } catch (error) {
      ...
      try {
        ...
        if (isMounted) {
          ...
          finalStores.forEach((store, index) => {
            console.log(`Nearby Store ${index + 1}:`, store.name, store.id, store.location);
          });

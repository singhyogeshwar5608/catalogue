...
=======
    /**
     * One slide per visible category: the first URL in {@see Category::banner_images},
     * or legacy {@see Category::banner_image} when the array is empty.
     */
    public function heroBanners(Request $request)
    {
        ...
        foreach ($categories as $category) {
            ...
            if ($urls === [] && $has('banner_image')) {
                $one = $category->getAttribute('banner_image');
            ...
            $titleBase = $has('banner_title') ? $category->getAttribute('banner_title') : null;
            $subtitle = $has('banner_subtitle') ? $category->getAttribute('banner_subtitle') : null;
            $name = $has('name') ? (string) $category->getAttribute('name') : 'Category';

            $title = is_string($titleBase) && trim($titleBase) !== ''
                ? trim($titleBase)
                : $name;
            $sub = is_string($subtitle) && trim($subtitle) !== ''
                ? trim($subtitle)
                : null;
